...
var fs = require("fs");
axios
  .get("http://saral.navgurukul.org/api/courses/74/exercises")
  .then((response) => {
...
var read = JSON.parse(fs.readFileSync("courses1.json", "utf-8")),
  int = require("prompt-sync")();
var read = read["data"],
  caunt = 0,
  caunt1 = 0;
for (i of read) {
...
for (j in arry) {
  if (input == j) {
    axios
      .get("http://saral.navgurukul.org/api/courses/" + arry[j] + "/exercises")
      .then((response) => {
